...
전략 패턴이란, 비슷한 동작을 하지만 다르게 구현되어 있는 행위들을 공통의 인터페이스를 구현하는 클래스들을 만들고, 동적으로 바꿀 수 있도록 하는 패턴이다. 전략 패턴으로 구현된 코드는 직접 행위에 대한 코드를 수정할 필요 없이 전략만 변경하여 유연하게 확장할 수 있게 된다.
